...
// This file contains the actual ZK proof implementation
// It uses eval to load snarkjs/circomlibjs at runtime to avoid SSR build errors

import type { ReputationProofInputs, ReputationProofResult } from './reputation-prover';
...
// Use indirect import to prevent webpack/turbopack from analyzing these at build time
async function loadSnarkjs() {
  if (!snarkjsModule) {
    const moduleName = 'snarkjs';
    snarkjsModule = await (Function('m', 'return import(m)')(moduleName));
  }
...
async function loadCircomlib() {
  if (!circomlibModule) {
    const moduleName = 'circomlibjs';
    circomlibModule = await (Function('m', 'return import(m)')(moduleName));
  }
